...
/**
 * Generate missing backend Resources + DTOs + panel TS contracts from Laravel Requests.
 *
 * Usage:
 *   php backend/scripts/generate-ai-contracts.php
 */
...
function inferPhpType(array $rules): string
{
    foreach ($rules as $rule) {
        if ($rule === 'array') return 'array';
    }
    foreach ($rules as $rule) {
        if ($rule === 'boolean') return 'bool';
    }
    foreach ($rules as $rule) {
        if ($rule === 'integer') return 'int';
    }
    foreach ($rules as $rule) {
        if ($rule === 'numeric') return 'float';
    }
    return 'string';
}
...
function inferTsType(array $rules): string
{
    ...
    foreach ($rules as $rule) {
        if ($rule === 'boolean') return 'boolean';
    }
    foreach ($rules as $rule) {
...
$modelFiles = array_merge(
    readPhpFiles($backendApp . '/Models'),
    readPhpFiles($domainBase)
);
...
foreach ($modelFiles as $mf) {
    ...
    if (in_array($class, $resourceClassBases, true)) {
        continue;
    }

    if ($class === 'OrderStatusHistory' && in_array('StatusHistory', $resourceClassBases, true)) {
        continue;
    }
...
foreach ($requestFiles as $reqFile) {
    ...
    if (!file_exists($dtoPath)) {
        ...
        file_put_contents($dtoPath, implode("\n", $lines));
...
$tsDir = $panelDir . '/app/types';
ensureDir($tsDir);
$tsPath = $tsDir . '/backend-contracts.generated.ts';
...
file_put_contents($tsPath, implode("\n", $ts));
...
fwrite(STDOUT, "Generated TS: {$tsPath}" . PHP_EOL);

if ($generatedResources) {
    fwrite(STDOUT, PHP_EOL . "Resources:" . PHP_EOL . implode(PHP_EOL, $generatedResources) . PHP_EOL);
}
if ($generatedDtos) {
    fwrite(STDOUT, PHP_EOL . "DTOs:" . PHP_EOL . implode(PHP_EOL, $generatedDtos) . PHP_EOL);
}
if ($patchedRequests) {
    fwrite(STDOUT, PHP_EOL . "Requests:" . PHP_EOL . implode(PHP_EOL, $patchedRequests) . PHP_EOL);
}
