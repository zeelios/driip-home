...
export const useAuthStore = defineStore('auth', {
  ...
  getters: {
    ...
    canWrite:        (s) => ['admin', 'manager', 'staff'].includes(s.staff?.role ?? ''),
  },

  actions: {
    async bootstrap () {
      const { getAccess, get, clearTokens } = useApi()
      if (!getAccess()) { this.status = 'guest'; return }
      this.status = 'loading'
      try {
    ...
    async login (email: string, password: string): Promise<boolean> {
      ...
      try {
        const res = await post<{ access_token: string; refresh_token: string; staff: StaffProfile }>(
