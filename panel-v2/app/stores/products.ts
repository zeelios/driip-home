...
export interface Product {
  ...
  created_at: string;
  updated_at: string;
  thumbnail_url?: string; // From ProductWithThumbnail response
}
  ...
  actions: {
    async fetchList() {
      ...
      try {
        ...
        // Response is now ProductWithThumbnail with flattened product + thumbnail_url
        const res = await get<any[]>("/products", p);
        // Each item contains all product fields + thumbnail_url
        this.list = Array.isArray(res) ? res : res.data ?? res.items ?? [];
        this.total = Array.isArray(res)
          ? res.length
          : res.meta?.total ?? this.list.length;
      } catch (e: any) {
        this.error = e?.data?.message ?? "Failed to load";
      } finally {
        this.loading = false;
      }
    },
    ...
    async fetchDetail(id: string) {
      ...
      const { get } = useApi();
      try {
        this.current = await get<Product>(`/products/${id}`);
      } catch (e: any) {
    ...
    async create(
      payload: Omit<
        Product,
        "id" | "stock_quantity" | "created_at" | "updated_at"
      >
    ) {
      ...
      try {
        ...
        this.error = e?.data?.message ?? "Failed to create";
        return null;
      } finally {
    ...
    async update(id: string, payload: Partial<Product>) {
      ...
      try {
        ...
        this.error = e?.data?.message ?? "Update failed";
        return false;
      } finally {
        this.actionBusy = false;
      }
    ...
    async remove(id: string) {
      ...
      try {
        ...
        this.error = e?.data?.message ?? "Delete failed";
        return false;
      } finally {
        this.actionBusy = false;
      }
