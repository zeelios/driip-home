  ...
  <div class="page">
    ...
    <div class="table-card">
      <ZTable
        ...
        row-key="id"
        :on-row-click="(r) => navigateTo(`/products/${r.id}`)"
      >
        ...
        <template #cell-price_cents="{ value }"
          ><span class="price">{{ formatVND(value) }}</span></template
        >
        <template #cell-stock_quantity="{ value }">
          <PBadge
            :status="value <= 0 ? 'danger' : value <= 5 ? 'pending' : 'ok'"
            :label="String(value)"
          />
        </template>
        <template #cell-actions="{ row }">
          <div class="row-actions">
            <PBtn variant="ghost" size="xs" @click.stop="openEdit(row)"
              >Sửa</PBtn
            >
            <PBtn
              variant="danger"
              size="xs"
              :loading="store.actionBusy && delId === row.id"
    ...
    >
      ...
      <template #footer>
        <PBtn
          ...
          >Hủy</PBtn
        >
        <PBtn :loading="store.actionBusy" @click="save">{{
          editingId ? "Lưu" : "Tạo"
        }}</PBtn>
      </template>
    </ZModal>
...
function openEdit(row: any) {
  ...
  Object.assign(form, {
    name: row.name,
    sku: row.sku,
    price_cents: String(row.price_cents / 100),
...
function resetForm() {
  editingId.value = "";
  Object.assign(form, { name: "", sku: "", price_cents: "", description: "" });
}
async function save() {
  const payload = {
    name: form.name,
