<template>
  <div class="page">
    ...
    <!-- NAV -->
    <nav class="nav">
      <NuxtLinkLocale to="/" class="nav-logo">driip<span class="dash">-</span></NuxtLinkLocale>
      <div class="nav-right">
        ...
        <div class="product-grid">
          ...
          <div class="product-card reveal" style="transition-delay: 0ms">
            <div class="product-img brief-gallery">
              <NuxtImg
                :src="`/products/Brief/${briefImg}.png`"
                :width="600"
                :height="750"
                format="webp"
                quality="85"
                fit="cover"
                :alt="`CK Brief ${briefImg}`"
                loading="lazy"
                class="product-img-el"
              />
              <button class="gallery-arrow gallery-prev" @click="briefImgPrev" aria-label="Previous image">‹</button>
              <button class="gallery-arrow gallery-next" @click="briefImgNext" aria-label="Next image">›</button>
            ...
            <div class="product-card-body">
              <div class="product-card-top">
                ...
                <span class="product-badge">{{ t('ck.products.from') }} RM 79</span>
              </div>
              <div class="product-divider"></div>
              <p class="product-desc">{{ t('ck.products.brief.desc') }}</p>
              <ul class="product-specs">
                <li v-for="spec in t('ck.products.brief.specs')" :key="spec">{{ spec }}</li>
              </ul>
          ...
          <div class="product-card reveal" style="transition-delay: 140ms">
            ...
            <div class="product-card-body">
              <div class="product-card-top">
                ...
                <span class="product-badge">{{ t('ck.products.from') }} RM 95</span>
              </div>
              <div class="product-divider"></div>
              <p class="product-desc">{{ t('ck.products.boxer.desc') }}</p>
              <ul class="product-specs">
                <li v-for="spec in t('ck.products.boxer.specs')" :key="spec">{{ spec }}</li>
              </ul>
    ...
    <section class="order-section" id="order">
      <div class="order-inner">
        ...
        <form v-if="orderState !== 'success'" class="order-form" @submit.prevent="submitOrder">
            ...
            <div class="order-fields">
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <input v-model="order.firstName" type="text" :placeholder="t('ck.order.firstNamePlaceholder')" required autocomplete="given-name" />
                </div>
                <div class="form-field">
                  <label>{{ t('ck.order.lastName') }}</label>
                  <input v-model="order.lastName" type="text" :placeholder="t('ck.order.lastNamePlaceholder')" required autocomplete="family-name" />
                </div>
              </div>
              ...
              <div class="form-row single">
                <div class="form-field">
                  ...
                  <input v-model="order.phone" type="tel" :placeholder="t('ck.order.phonePlaceholder')" required autocomplete="tel" />
                </div>
              </div>
              ...
              <div class="form-row single">
                <div class="form-field">
                  ...
                  <select v-model="order.province" @change="onProvinceChange" required>
                    <option value="" disabled>{{ t('ck.order.provincePlaceholder') }}</option>
                    <option v-for="prov in vietnamProvinces" :key="prov.code" :value="prov.name">
                      {{ prov.name }}
                    </option>
                  </select>
                </div>
              ...
              <div class="form-row single">
                <div class="form-field">
                  ...
                  <select
                    v-if="selectedProvince && selectedProvince.districts.length > 0"
                    v-model="order.district"
                    required
                  >
                    <option value="" disabled>{{ t('ck.order.districtPlaceholder') }}</option>
                    <option v-for="dist in selectedProvince.districts" :key="dist" :value="dist">
                      {{ dist }}
                    </option>
                  </select>
                  <input
              ...
              <div class="form-row single">
                <div class="form-field">
                  ...
                  <input v-model="order.ward" type="text" :placeholder="t('ck.order.wardPlaceholder')" required />
                </div>
              </div>
              ...
              <div class="form-row single">
                <div class="form-field">
                  ...
                  <input v-model="order.street" type="text" :placeholder="t('ck.order.streetPlaceholder')" required autocomplete="street-address" />
                </div>
              </div>
          ...
          <div class="order-group">
            ...
            <div class="order-fields">
              ...
              <div class="select-group">
                <label class="select-group-label">{{ t('ck.order.product') }}</label>
              ...
              <div class="select-group">
                <label class="select-group-label">
                  {{ t('ck.order.colorPack') }} <span class="opt">{{ t('ck.order.colorPackSub') }}</span>
                </label>
                <div class="color-selection-area">
                  ...
                  <!-- Order preview image -->
                  <div v-if="order.sku && order.color" class="order-preview-img">
                    <NuxtImg
          ...
          <!-- Validation hint -->
          <p v-if="orderValidationMsg" class="order-validation">{{ orderValidationMsg }}</p>

          <div v-if="orderState === 'error'" class="form-error">{{ t('common.error') }}</div>

          <!-- Summary bar -->
          <div v-if="order.sku && order.size && order.color" class="order-summary">
            <div class="summary-row">
              <span>{{ skuLabel }} · {{ order.size }} · {{ colorLabel }}</span>
              <span class="summary-price">RM {{ orderPrice }} <s class="summary-original">RM {{ Math.round(orderPrice / 0.8) }}</s></span>
            </div>
            <div class="summary-coupon">{{ t('ck.order.codeApplied') }}</div>
          </div>

          <button type="submit" class="btn-order-submit" :disabled="orderState === 'loading'">
            <span v-if="orderState === 'idle' || orderState === 'error'">
              {{ t('ck.order.submit', { price: orderPrice }) }}
            </span>
            <span v-else class="loading-dots dark"><span></span><span></span><span></span></span>
          </button>

          <p class="form-fine">{{ t('ck.order.fine') }}</p>
        </form>

        <div v-if="orderState === 'success'" class="success-message order-success">
          <div class="success-icon">✓</div>
    ...
    <footer class="footer">
      <div class="footer-inner">
        ...
        <div class="footer-links">
          <a href="https://www.facebook.com/profile.php?id=61586812299701" target="_blank" rel="noopener">
            {{ t('footer.facebook') }}
          </a>
        </div>
...
useHead({
  title: computed(() => locale.value === 'vi'
...
const colorOptions = computed(() => [
  { value: '3x-black', label: t('ck.order.colors.black'), swatches: ['#111', '#111', '#111'] },
  { value: '3x-white', label: t('ck.order.colors.white'), swatches: ['#f0f0f0', '#f0f0f0', '#f0f0f0'] },
  { value: '3x-grey',  label: t('ck.order.colors.grey'),  swatches: ['#888', '#888', '#888'] },
  { value: 'mix',      label: t('ck.order.colors.mix'),   swatches: ['#111', '#888', '#f0f0f0'] },
])
...
const orderPreviewColor = computed(() =>
  colorToImage[order.color] ?? 'Black',
)

const orderPrice = computed(() => {
  const base = order.sku === 'ck-brief' ? 79 : order.sku === 'ck-boxer' ? 95 : 89
...
function setupParallax() {
  ...
  const onScroll = () => root.style.setProperty('--scroll-y', window.scrollY.toString())
...
function briefImgPrev() {
  briefImg.value = briefImg.value === 1 ? 3 : briefImg.value - 1
}

function briefImgNext() {
  briefImg.value = briefImg.value === 3 ? 1 : briefImg.value + 1
}
...
async function submitAccess() {
  ...
  try {
    ...
    setTimeout(() => { accessState.value = 'idle' }, 3000)
  }
}
...
async function submitOrder() {
  ...
  try {
    ...
    setTimeout(() => { orderState.value = 'idle' }, 3000)
  }
}
</script>
...
.nav-logo { font-family: var(--font-display); font-size: 24px; letter-spacing: 0.1em; color: var(--white); text-decoration: none; }
.nav-right { display: flex; align-items: center; gap: 16px; }
.lang-switch {
  font-size: 10px; font-weight: 600; letter-spacing: 0.25em;
  color: var(--grey-400); border: 1px solid var(--grey-700); padding: 4px 10px;
  background: transparent; cursor: pointer; transition: color 0.2s, border-color 0.2s;
}
...
.hero-pre { font-size: 11px; font-weight: 500; letter-spacing: 0.3em; color: var(--grey-400); margin-bottom: 28px; }

.hero-title { display: flex; flex-direction: column; line-height: 0.88; margin-bottom: 36px; }
...
.hero-sub {
  font-size: clamp(12px, 2vw, 15px); font-weight: 300; letter-spacing: 0.25em;
  color: var(--grey-400); text-transform: uppercase; line-height: 1.8; margin-bottom: 52px;
  border-left: 1px solid var(--grey-700); padding-left: 16px;
...
.btn-primary {
  ...
  padding: 16px 32px; font-family: var(--font-body); font-size: 12px;
  font-weight: 600; letter-spacing: 0.2em; cursor: pointer;
  transition: background 0.2s, gap 0.2s;
}
.btn-primary:hover { background: var(--off-white); gap: 20px; }
...
.parallax-bg {
  ...
  letter-spacing: -0.05em; pointer-events: none; user-select: none; line-height: 1;
}
...
.promo-strip {
  ...
  font-size: 10px; font-weight: 600; letter-spacing: 0.25em; white-space: nowrap;
}
.promo-strip .dot { color: #aaa; flex-shrink: 0; }

/* ─── PRODUCTS ─────────────────────────────────────────────────── */
.products { background: var(--black); padding: 80px 24px; }
.products-inner { max-width: 1100px; margin: 0 auto; }

.products-title {
  font-family: var(--font-display);
  font-size: clamp(48px, 9vw, 96px);
  line-height: 0.9; color: var(--white); letter-spacing: -0.01em; margin-bottom: 64px;
  white-space: pre-line;
}
...
.product-card:hover .product-img-el { transform: scale(1.04); }

/* Brief gallery controls */
.brief-gallery { position: relative; }

.gallery-arrow {
  position: absolute; top: 50%; transform: translateY(-50%);
  z-index: 4; background: rgba(0,0,0,0.55); color: var(--white);
  border: none; width: 36px; height: 52px;
  display: flex; align-items: center; justify-content: center;
  font-size: 22px; cursor: pointer; opacity: 0;
  transition: opacity 0.2s, background 0.2s;
}
.brief-gallery:hover .gallery-arrow { opacity: 1; }
.gallery-prev { left: 0; }
...
.gallery-dot.active { background: var(--white); transform: scale(1.25); }
...
.boxer-swatch.active { border-color: var(--white); transform: scale(1.2); }
.boxer-swatch:hover:not(.active) { border-color: rgba(255,255,255,0.7); }
...
.product-divider { width: 32px; height: 1px; background: var(--grey-700); margin-bottom: 14px; }

.product-desc {
  font-size: 14px; font-weight: 300; color: var(--grey-400);
  line-height: 1.7; margin-bottom: 16px; max-width: 440px;
}

.product-specs { list-style: none; display: flex; flex-direction: column; gap: 6px; margin-bottom: 24px; }
.product-specs li {
  font-size: 11px; font-weight: 500; letter-spacing: 0.15em; color: var(--grey-700);
  display: flex; align-items: center; gap: 8px;
}
.product-specs li::before { content: ''; width: 16px; height: 1px; background: var(--grey-700); flex-shrink: 0; }
...
.btn-order-now:hover { border-color: var(--white); background: rgba(255,255,255,0.05); }
...
.fabric-bar {
  ...
  gap: 12px 20px; padding: 20px 24px;
  border-top: 1px solid rgba(255,255,255,0.06);
  border-bottom: 1px solid rgba(255,255,255,0.06);
}
.fabric-item { font-size: 10px; font-weight: 600; letter-spacing: 0.25em; color: var(--grey-400); }
.fabric-sep  { color: var(--grey-700); font-size: 12px; }

/* ─── OFFER ────────────────────────────────────────────────────── */
.offer { background: var(--off-white); color: var(--black); padding: 80px 24px; }
.offer-inner { max-width: 1100px; margin: 0 auto; display: grid; grid-template-columns: 1fr; gap: 48px; }
...
.offer-title {
  font-family: var(--font-display); font-size: clamp(52px, 10vw, 88px);
  line-height: 0.95; letter-spacing: -0.01em; color: var(--black); margin-bottom: 20px;
  white-space: pre-line;
}
.offer-body  { font-size: 14px; font-weight: 300; color: var(--grey-700); line-height: 1.7; max-width: 380px; }

.coupon-label { font-size: 10px; font-weight: 600; letter-spacing: 0.3em; color: var(--grey-700); margin-bottom: 12px; }

.coupon-box { display: flex; align-items: stretch; border: 1.5px solid var(--black); margin-bottom: 12px; }
.coupon-code {
  font-family: var(--font-display); font-size: clamp(28px, 6vw, 44px);
  letter-spacing: 0.15em; padding: 16px 24px; flex: 1;
  background: var(--black); color: var(--white); display: flex; align-items: center;
}
.copy-btn {
  padding: 0 24px; background: transparent; border: none;
  border-left: 1.5px solid var(--black); font-family: var(--font-body);
  font-size: 11px; font-weight: 600; letter-spacing: 0.2em;
  cursor: pointer; color: var(--black); transition: background 0.15s, color 0.15s; min-width: 90px;
}
.copy-btn:hover, .copy-btn.copied { background: var(--black); color: var(--white); }
.coupon-note { font-size: 11px; color: var(--grey-400); letter-spacing: 0.05em; }

/* ─── MANIFESTO ────────────────────────────────────────────────── */
.manifesto { background: var(--black); padding: 100px 24px; text-align: center; }
.manifesto-inner { max-width: 700px; margin: 0 auto; }
.manifesto-text {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: clamp(26px, 5vw, 44px); font-weight: 300;
  line-height: 1.65; color: var(--grey-100); letter-spacing: 0.02em;
  white-space: pre-line;
}
...
.access-title {
  ...
  line-height: 0.95; letter-spacing: -0.01em; color: var(--white); margin-bottom: 20px;
  white-space: pre-line;
}
...
/* ─── ORDER SECTION ────────────────────────────────────────────── */
.order-section {
  background: var(--black); padding: 80px 24px 120px;
...
.order-title {
  ...
  line-height: 0.9; color: var(--white); letter-spacing: -0.01em; margin-bottom: 20px;
}
.order-sub { font-size: 14px; font-weight: 300; color: var(--grey-400); line-height: 1.7; }

.order-form { display: flex; flex-direction: column; gap: 0; }
.order-group { margin-bottom: 0; }

.order-group-label {
  font-size: 10px; font-weight: 600; letter-spacing: 0.3em;
  color: var(--grey-700); padding: 28px 0 0;
  border-top: 1px solid rgba(255,255,255,0.08);
  display: block; margin-bottom: 0;
}
.order-fields { padding: 0; }

/* ─── FORM SHARED ──────────────────────────────────────────────── */
.form, .order-form { display: flex; flex-direction: column; }
...
.form-field select {
  ...
  color: var(--white); padding: 4px 0; resize: vertical;
  appearance: none; -webkit-appearance: none;
}
.form-field input::placeholder,
.form-field textarea::placeholder { color: var(--grey-700); }
.form-field select option { background: #111; color: white; }

/* Placeholder style for select when empty */
.form-field select:invalid,
.form-field select option[disabled] { color: var(--grey-700); }

.form-error {
  margin: 16px 0; padding: 12px 16px;
  border: 1px solid rgba(255,80,80,0.4); color: #ff6b6b;
...
.select-group-label {
  font-size: 10px; font-weight: 600; letter-spacing: 0.25em;
  color: var(--grey-400); display: block; margin-bottom: 16px;
}
...
.select-tile:hover { border-color: var(--grey-400); }
.select-tile.active { border-color: var(--white); background: rgba(255,255,255,0.04); }
...
.tile-price {
  font-size: 11px; font-weight: 500; letter-spacing: 0.15em; color: var(--grey-400);
}

/* Size pills */
.select-pills { display: flex; flex-wrap: wrap; gap: 8px; }
...
.select-pill {
  ...
  letter-spacing: 0.1em; color: var(--grey-400); cursor: pointer;
  transition: border-color 0.15s, color 0.15s, background 0.15s;
}
.select-pill:hover { border-color: var(--grey-400); color: var(--white); }
.select-pill.active { border-color: var(--white); color: var(--white); background: rgba(255,255,255,0.06); }

/* Colour tiles + preview */
.color-selection-area { display: flex; flex-direction: column; gap: 20px; }

.color-tiles { grid-template-columns: 1fr 1fr; }
...
.order-validation {
  margin: 12px 0 0;
  font-size: 11px; color: #ff9f6b; letter-spacing: 0.1em;
}
...
.summary-row {
  ...
  font-size: 13px; font-weight: 500; letter-spacing: 0.1em; color: var(--white);
  margin-bottom: 6px;
}

.summary-price { font-family: var(--font-display); font-size: 22px; letter-spacing: 0.05em; }

.summary-original { font-size: 14px; color: var(--grey-700); text-decoration: line-through; margin-left: 8px; }

.summary-coupon {
  font-size: 10px; font-weight: 600; letter-spacing: 0.2em; color: var(--grey-400);
}

/* ─── SUBMIT BUTTONS ───────────────────────────────────────────── */
.btn-submit, .btn-order-submit {
...
.btn-order-submit:hover:not(:disabled) { background: var(--off-white); }
.btn-submit:disabled,
.btn-order-submit:disabled { opacity: 0.5; cursor: not-allowed; }

.form-fine {
  margin-top: 16px; font-size: 10px; color: var(--grey-700);
  text-align: center; letter-spacing: 0.05em; line-height: 1.6;
}

/* Loading dots */
.loading-dots { display: inline-flex; align-items: center; gap: 4px; }
.loading-dots span {
  width: 5px; height: 5px; background: var(--black);
  border-radius: 50%; animation: bounce 0.6s infinite alternate;
}
.loading-dots.dark span { background: var(--black); }
...
/* ─── FOOTER ───────────────────────────────────────────────────── */
.footer { background: var(--black); border-top: 1px solid rgba(255,255,255,0.08); padding: 32px 24px; }
...
.footer-logo { font-family: var(--font-display); font-size: 28px; letter-spacing: 0.1em; }
.footer-links { display: flex; gap: 24px; }
.footer-links a { font-size: 10px; font-weight: 600; letter-spacing: 0.25em; color: var(--grey-400); text-decoration: none; transition: color 0.2s; }
.footer-links a:hover { color: var(--white); }
.footer-copy { font-size: 9px; color: var(--grey-700); letter-spacing: 0.2em; }
...
@media (min-width: 640px) {
  ...
  .offer-inner      { grid-template-columns: 1fr 1fr; align-items: start; }
  .select-tiles     { grid-template-columns: 1fr 1fr; }
  .color-tiles      { grid-template-columns: repeat(4, 1fr); }
  .color-selection-area { flex-direction: row; align-items: flex-start; gap: 28px; }
  .color-selection-area .select-tiles { flex: 1; }

  .form-row:not(.single) { grid-template-columns: 1fr 1fr; }
  .form-row:not(.single) .form-field { border-bottom: none; }
  .form-row:not(.single) .form-field:first-child { border-right: 1px solid rgba(255,255,255,0.1); padding-right: 32px; }
  .form-row:not(.single) .form-field:last-child  { padding-left: 32px; }
...
@media (min-width: 1024px) {
  ...
  .products       { padding: 100px 64px; }
  .offer          { padding: 100px 64px; }
  .manifesto      { padding: 140px 64px; }
  .access         { padding: 100px 64px 120px; }
  .order-section  { padding: 100px 64px 140px; }
  .footer         { padding: 40px 64px; }

  .promo-strip {
    justify-content: center; gap: 32px;
    font-size: 11px; padding: 14px 64px;
  }
  .fabric-bar { flex-wrap: nowrap; padding: 20px 64px; gap: 28px; }
