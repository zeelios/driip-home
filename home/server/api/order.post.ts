...
export default defineEventHandler(async (event) => {
  ...
  if (!firstName || !lastName || !phone || !province || !(fullAddress || street)) {
    throw createError({
      statusCode: 400,
  ...
  const items: CartItemPayload[] = Array.isArray(cartItems) && cartItems.length > 0
    ? cartItems
    : [{ sku, size, color, boxes: Number(boxes) || 1, finalTotal: finalTotal || 0, compareTotal: 0 }];
  ...
  try {
    ...
    for (const item of items) {
      ...
      const itemFinalTotal = quantity > 0 ? item.finalTotal / quantity : item.finalTotal;
