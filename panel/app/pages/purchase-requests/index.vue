<template>
  <div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
      <div
        class="bg-[#111111] border border-amber-500/20 rounded-lg p-2.5 sm:p-3"
      >
      ...
      <div
        class="bg-[#111111] border border-red-500/20 rounded-lg p-2.5 sm:p-3"
      >
        <p class="text-[11px] sm:text-xs text-white/50 mb-0.5 sm:mb-1">
          Đơn thiếu hàng
        </p>
        <p class="text-lg sm:text-xl font-bold text-red-500">
      ...
      <div class="bg-[#111111] border border-white/8 rounded-lg p-2.5 sm:p-3">
        ...
        <p class="text-lg sm:text-xl font-bold text-white/90">
          {{ summary.total_items_needing_purchase }}
        </p>
      </div>
      <div class="bg-[#111111] border border-white/8 rounded-lg p-2.5 sm:p-3">
        <p class="text-[11px] sm:text-xs text-white/50 mb-0.5 sm:mb-1">
        ...
        <p class="text-lg sm:text-xl font-bold text-white/90">
          {{ formatVndCompact(summary.estimated_cost) }}
        </p>
      </div>
    </div>
    ...
    <div
      class="flex items-center gap-1 mb-4 border-b border-white/10 overflow-x-auto scrollbar-none"
    >
      <button
        v-for="tab in tabs"
        :key="tab.key"
      ...
      >
        {{ tab.label }}
        <span
          v-if="tab.key === 'low-stock' && summary.low_stock_count > 0"
          class="ml-1 text-xs text-amber-500"
        >
          ({{ summary.low_stock_count }})
        </span>
        <span
          v-if="tab.key === 'unfulfillable' && summary.unfulfillable_count > 0"
    ...
    <div class="flex flex-col sm:flex-row gap-2 mb-4">
      ...
      <ZButton
        size="sm"
        variant="outline"
        @click="navigateTo('/purchase-orders/new')"
      ...
      >
        ...
        Tạo đơn đặt hàng ({{ selectedItems.length }})
      </ZButton>
    </div>
    ...
    <div v-if="activeTab === 'low-stock'" class="block sm:hidden space-y-2">
      ...
      >
        <div class="flex items-start gap-2 mb-2">
          <ZCheckbox
            ...
            size="md"
            @update:model-value="toggleSelectLowStock(item)"
            @click.stop
          />
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-white/85 truncate">
              {{ item.product_name }}
            </p>
            <p class="text-xs text-white/50 font-mono">{{ item.sku }}</p>
          </div>
          <span
            class="text-xs text-white/70 bg-white/10 px-2 py-0.5 rounded whitespace-nowrap"
            >{{ item.warehouse_name }}</span
          >
        </div>

        <div
          class="grid grid-cols-3 gap-2 text-center py-2 border-y border-white/5"
        >
          <div>
            ...
            <p
              ...
              :class="
                item.quantity_available <= 0 ? 'text-red-500' : 'text-amber-500'
              "
            >
        ...
        <div class="flex items-center justify-between pt-2">
          <span class="text-xs text-white/50">{{
            item.supplier?.sku ?? "Chưa có NCC"
          }}</span>
          <span class="text-xs text-white/70"
            >{{ formatVnd(item.unit_cost) }}/sp</span
          >
        </div>
      </div>
      ...
      <!-- Mobile Load More -->
      <div
        v-if="lowStockMeta.current_page < lowStockMeta.last_page"
        class="flex justify-center mt-3"
      >
        <ZButton
          size="sm"
          variant="outline"
          @click="loadMoreLowStock"
      ...
      <p class="text-center text-xs text-white/40 mt-2">
        {{ lowStockItems.length }} / {{ lowStockMeta.total }} mục
      </p>
    </div>
      ...
      >
        <template #cell-sku="{ row }">
          <span class="font-mono text-sm text-white/70">{{
            (row as LowStockItem).sku
          }}</span>
        </template>
        <template #cell-product="{ row }">
          <span class="font-medium text-white/85">{{
            (row as LowStockItem).product_name
          }}</span>
        </template>
        <template #cell-stock="{ row }">
          <div class="flex flex-col text-xs">
            <span
              :class="(row as LowStockItem).quantity_available <= 0 ? 'text-red-500' : 'text-amber-500'"
            >
              Có sẵn: {{ (row as LowStockItem).quantity_available }}
            </span>
            <span class="text-white/40"
              >Tồn kho: {{ (row as LowStockItem).quantity_on_hand }}</span
            >
          </div>
        </template>
        <template #cell-reorder="{ row }">
          <span class="text-sm text-white/70">{{
            (row as LowStockItem).reorder_point
          }}</span>
        </template>
        <template #cell-suggested="{ row }">
          <span class="text-sm font-semibold text-white/90">{{
            (row as LowStockItem).suggested_quantity
          }}</span>
        </template>
        <template #cell-cost="{ row }">
          <span class="text-sm text-white/70">{{
            formatVnd((row as LowStockItem).unit_cost)
          }}</span>
        </template>
        <template #cell-supplier="{ row }">
          <span class="text-xs text-white/50">{{
            (row as LowStockItem).supplier?.sku ?? "—"
          }}</span>
        </template>
      </ZTable>
      <div class="flex items-center justify-between gap-3 pt-4 flex-wrap">
        <p class="m-0 text-[0.8125rem] text-white/40">
          {{ lowStockMeta.total }} mục
        </p>
        <ZPagination
          :current-page="lowStockMeta.current_page"
          :total-pages="lowStockMeta.last_page"
          @change="onLowStockPageChange"
        />
      </div>
    </div>
    ...
    <div v-if="activeTab === 'unfulfillable'" class="block sm:hidden space-y-2">
      <div
        ...
        class="bg-[#111111] border border-white/8 rounded-lg p-3"
        :class="{
          'border-red-500/30 bg-red-500/5': isSelectedUnfulfillable(item),
        }"
      >
        <div class="flex items-start gap-2 mb-2">
          <ZCheckbox
            :model-value="isSelectedUnfulfillable(item)"
            size="md"
            @update:model-value="toggleSelectUnfulfillable(item)"
            @click.stop
          />
          <div class="flex-1 min-w-0">
            <NuxtLink
      ...
      <!-- Mobile Load More -->
      <div
        v-if="unfulfillableMeta.current_page < unfulfillableMeta.last_page"
        class="flex justify-center mt-3"
      >
        <ZButton
          size="sm"
          variant="outline"
          @click="loadMoreUnfulfillable"
      ...
      <p class="text-center text-xs text-white/40 mt-2">
        {{ unfulfillableItems.length }} / {{ unfulfillableMeta.total }} mục
      </p>
    </div>
    ...
    <div v-if="activeTab === 'unfulfillable'" class="hidden sm:block">
      ...
      >
        <template #cell-order="{ row }">
          <NuxtLink
            :to="`/orders/${(row as UnfulfillableItem).order_id}`"
            class="font-medium text-white/90 hover:text-blue-400 block truncate"
          >
        ...
        <template #cell-product="{ row }">
          <div class="flex flex-col">
            <span class="font-medium text-white/85">{{
              (row as UnfulfillableItem).product_name
            }}</span>
            <span class="text-xs font-mono text-white/50">{{
              (row as UnfulfillableItem).sku
            }}</span>
          </div>
        </template>
        <template #cell-size="{ row }">
          <span class="text-sm text-white/70">{{
            (row as UnfulfillableItem).size_display ?? "—"
          }}</span>
        </template>
        <template #cell-customer="{ row }">
          <span class="text-sm text-white/70">{{
            (row as UnfulfillableItem).customer_name
          }}</span>
        </template>
        <template #cell-order-date="{ row }">
          <span class="text-sm text-white/50">{{
            formatDatetime((row as UnfulfillableItem).order_date)
          }}</span>
        </template>
        <template #cell-cost="{ row }">
          <span class="text-sm text-white/70">{{
            formatVnd((row as UnfulfillableItem).unit_cost)
          }}</span>
        </template>
      </ZTable>
      <div class="flex items-center justify-between gap-3 pt-4 flex-wrap">
        <p class="m-0 text-[0.8125rem] text-white/40">
          {{ unfulfillableMeta.total }} mục
        </p>
        <ZPagination
          :current-page="unfulfillableMeta.current_page"
          :total-pages="unfulfillableMeta.last_page"
          @change="onUnfulfillablePageChange"
        />
      </div>
    </div>
      ...
      >
        <div class="p-3 border-b border-white/5 bg-white/5">
          <div class="flex items-center justify-between">
            <div>
              <p class="font-medium text-white/90 text-sm">
                NCC: {{ group.supplier_id.substring(0, 8) }}...
              </p>
              <p class="text-xs text-white/50">
            ...
            <div class="text-right">
              <p class="text-sm font-semibold text-white/90">
                {{ formatVndCompact(group.total_cost) }}
              </p>
            </div>
          </div>
        </div>
        <div class="p-2 space-y-1">
          ...
          >
            ...
            <span class="text-white/50 shrink-0"
              >{{ item.quantity }} x
              {{ formatVndCompact(item.unit_cost) }}</span
            >
          </div>
          <div v-if="group.items.length > 3" class="text-center py-1">
            <span class="text-xs text-white/40"
              >+{{ group.items.length - 3 }} sản phẩm khác</span
            >
          </div>
        </div>
        <div class="p-2 border-t border-white/5">
          <ZButton
            size="sm"
            variant="outline"
            @click="createPOForSupplier(group)"
            class="w-full"
          >
    ...
    <div v-if="activeTab === 'by-supplier'" class="hidden sm:block">
      ...
      <div
        v-for="group in supplierGroups"
        :key="group.supplier_id"
        class="mb-4 bg-[#111111] border border-white/8 rounded-lg p-4"
      >
        <div class="flex items-center justify-between mb-3">
          <div>
            <p class="font-medium text-white/90">
              Nhà cung cấp: {{ group.supplier_id.substring(0, 8) }}...
            </p>
            <p class="text-xs text-white/50">
              {{ group.items.length }} sản phẩm · Tổng:
              {{ formatVnd(group.total_cost) }}
            </p>
          </div>
          <ZButton
            size="sm"
        ...
        <div class="space-y-1">
          <div
            ...
            :key="item.product_id"
            class="flex justify-between py-1 text-sm border-b border-white/5 last:border-0"
          >
            <span class="text-white/70">{{ item.sku }} - {{ item.name }}</span>
            <span class="text-white/50"
              >{{ item.quantity }} x {{ formatVnd(item.unit_cost) }}</span
            >
          </div>
        </div>
      </div>
    ...
    <ZModal v-model="showCreatePOModal" title="Tạo đơn đặt hàng (PO)" size="md">
      <div class="flex flex-col gap-4">
        <p class="text-sm text-white/70">
          Tạo PO cho {{ selectedItems.length }} sản phẩm đã chọn
        </p>
        <div class="bg-white/5 rounded-lg p-3 max-h-60 overflow-y-auto">
          ...
          >
            <span class="text-white/70 truncate flex-1 mr-2">{{
              (item as any).sku || (item as any).product_name
            }}</span>
            <span class="text-white/50 shrink-0">{{
              (item as any).suggested_quantity ||
              (item as any).quantity_needed ||
              1
            }}</span>
          </div>
          <div v-if="selectedItems.length > 10" class="text-center py-2">
            <span class="text-xs text-white/40"
              >+{{ selectedItems.length - 10 }} sản phẩm khác</span
            >
          </div>
        </div>
      </div>
...
import { usePendingPOStore } from "~/stores/pending-po";
...
const unfulfillableMeta = ref<ListMeta>({
  current_page: 1,
  last_page: 1,
  total: 0,
});
...
const unfulfillableColumns: TableColumn[] = [
  { key: "selection", type: "selection", width: "40px" },
  { key: "order", label: "Đơn hàng", width: "140px" },
  { key: "product", label: "Sản phẩm" },
...
async function fetchSummary(): Promise<void> {
  try {
    const response = await api.get<{ data: typeof summary }>(
      "/purchase-requests"
    );
...
async function fetchLowStock(): Promise<void> {
  ...
  try {
    ...
    const response = await api.get<{ data: LowStockItem[]; meta: ListMeta }>(
      `/purchase-requests/low-stock?${params}`
    );
...
async function fetchUnfulfillable(): Promise<void> {
  ...
  try {
    ...
    const response = await api.get<{
      data: UnfulfillableItem[];
      meta: ListMeta;
    }>(`/purchase-requests/unfulfillable?${params}`);
    unfulfillableItems.value = response.data ?? [];
    unfulfillableMeta.value = response.meta ?? {
      current_page: 1,
      last_page: 1,
      total: 0,
    };
...
async function fetchSupplierGroups(): Promise<void> {
  try {
    const response = await api.get<{ data: SupplierGroup[] }>(
      "/purchase-requests/by-supplier"
    );
...
function navigateToCreatePO(): void {
  const items = selectedItems.value;
  if (items.length === 0) return;

  // Transform items to PendingPOItem format and save to store
  const poItems = items.map((item) => ({
    id: item.id,
    product_id: (item as any).product_id || item.id,
    sku: (item as any).sku || "",
    product_name: (item as any).product_name || "",
    size_option_id: (item as any).size_option_id || null,
    size_display: (item as any).size_display || null,
    color: (item as any).color || null,
    quantity_needed:
      (item as any).suggested_quantity || (item as any).quantity_needed || 1,
    quantity_to_order:
      (item as any).suggested_quantity || (item as any).quantity_needed || 1,
    unit_cost: (item as any).unit_cost || 0,
    warehouse_id: (item as any).warehouse_id,
    warehouse_name: (item as any).warehouse_name,
  }));

  const pendingPOStore = usePendingPOStore();
  pendingPOStore.setItems(
    poItems,
    activeTab.value === "low-stock" ? "low_stock" : "unfulfillable"
  );
  pendingPOStore.persistToStorage();

  navigateTo("/purchase-orders/new");
}
